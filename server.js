...
var staticServer = new nodeStaticAlias.Server(WEB_DIR, {
  ...
  alias: [
    ...
    {
      // basic static page friendly URL rewrites
      match: /^\/(?:contact|login|404|offline)(?:[#?]|$)/,
      serve: "<% basename %>.html",
      force: true
    },
    {
    ...
    staticServer.serve(req, res, function onStaticComplete(err) {
      if (err) {
        if (req.headers["accept"].includes("text/html")) {
          serveFile("/404.html", 200, { "X-Not-Found": "1" }, req, res).catch(
            console.error
          );
        } else {
          res.writeHead(404);
...
async function addPost(newPostData, req, res) {
  if (newPostData.title.length > 0 && newPostData.post.length > 0) {
    let postTemplate = await fsReadFile(
      path.join(WEB_DIR, "posts", "post.html"),
      "utf-8"
    );
    ...
    let [, year, month, day] = new Date()
      ...
      .match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (postIDs.length > 0) {
      let [, latestYear, latestMonth, latestDay, latestCount] = String(
        postIDs[0]
      ).match(/^(\d{4})(\d{2})(\d{2})(\d+)/);
    ...
    try {
      await fsWriteFile(
        path.join(WEB_DIR, "posts", `${newPostID}.html`),
        newPost,
        "utf8"
      );
...
function clearSession(req, res) {
  var sessionID =
    req.sessionID || (req.headers.cookie && req.headers.cookie.sessionId);
  ...
  res.setHeader(
    "Set-Cookie",
    getCookieHeaders(null, new Date(0).toUTCString())
  );
}
